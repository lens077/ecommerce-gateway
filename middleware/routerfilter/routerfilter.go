...
func NewPathMatcher(pattern string, methods []string) (*PathMatcher, error) {
	...
	// 处理没有斜杠的通配符，如 /user*
	isWildcard := strings.HasSuffix(pattern, "*") && !strings.HasSuffix(pattern, "/*") && !strings.HasSuffix(pattern, "/**")
	if isWildcard {
		// 对于没有斜杠的通配符，如 /user*，直接使用更简单的正则表达式
		basePattern := strings.TrimSuffix(pattern, "*")
		regexPattern = "^" + regexp.QuoteMeta(basePattern) + ".*$"
	} else {
		// 正常处理其他通配符
